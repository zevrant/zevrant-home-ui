...
CMD  echo "aws_access_key_id = $ACCESS_KEY" >> ~/.aws/credentials\
 ...
 && IP=$(hostname -I)\
 && read -ra arr <<< "$IP"\
 && IP_ADDRESS=${arr[2]} java -jar -Dspring.profiles.active=prod -Deureka.instance.hostname=$IP_ADDRESS /usr/local/microservices/zevrant-home-services/zevrant-home-ui/zevrant-home-ui.jar
